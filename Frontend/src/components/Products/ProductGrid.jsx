...
const ProductGrid = ({ products, loading, error, oneRowOnMobile = false }) => {
  ...
  const layoutClassName = oneRowOnMobile
    ? 'flex gap-2 overflow-x-auto px-1 pb-2 pr-1 md:grid md:grid-cols-3 lg:grid-cols-4 md:gap-2 md:overflow-visible md:px-0 md:pb-0 md:pr-0'
    : 'grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1 md:gap-2';

  const cardClassName = oneRowOnMobile
    ? 'group block flex-shrink-0 w-[88vw] max-w-[340px] snap-start bg-white rounded-2xl p-2.5 md:w-auto'
    : 'group block bg-white rounded-2xl p-2.5';
      ...
      <svg width="0" height="0" style={{ position: 'absolute', overflow: 'hidden' }}>
        <defs>
          <clipPath id="bucketClip" clipPathUnits="objectBoundingBox">
            {/* Square container, bucket shape: straight top, sides taper slightly, small smooth curve at bottom corners only */}
            {/*
              Rounded top-left → straight top → rounded top-right
              Straight sides tapering slightly inward
              Smooth small curve at bottom corners only
            */}
            <path d="
              M 0.06,0
              Q 0,0 0,0.06
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  className="absolute top-2.5 right-2.5 flex h-8 w-8 items-center justify-center rounded-full transition-transform duration-200 hover:scale-110"
                ...
                >
                  <svg
                    ...
                    width="20"
                    height="20"
                    stroke={isWishlisted(product._id) ? '#d64545' : '#ffffff'}
                    strokeWidth="2"
                ...
                <button
                  ...
                  className="absolute bottom-3 right-3 flex h-9 w-9 items-center justify-center rounded-full transition-transform duration-200 hover:scale-110"
                  onClick={(event) => handleCartClick(event, product._id)}
                >
                  <svg xmlnsXlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36" fill="none" aria-hidden="true">
                ...
                {/* image-level rating badge removed; using inline rating only */}
              </div>

              {/* Product info */}
              <div className="pt-1.5 pb-0.5">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-product text-[14px] font-medium truncate leading-tight text-gray-900">
                    {product.name}
                  </h3>
                  {displayRating > 0 && (
                    <div className="flex shrink-0 items-center gap-0.5 text-[11px] font-medium text-[#8b4fd1]">
                      <svg viewBox="0 0 24 24" width="12" height="12" fill="#8b4fd1" stroke="#8b4fd1" strokeWidth="1.4" aria-hidden="true">
                ...
                <div className="mt-1 flex items-center gap-2 flex-wrap">
                  ...
                  {discount && (
                    <>
                      <span className="line-through text-[11px] text-gray-400 leading-none">
                        ₹{product.price}
                      </span>
                      <span
